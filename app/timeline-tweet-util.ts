...
import { sanitizeTweetHtml } from '../lib/sanitize';
...
export function reconstructTweetHtml(
  ...
  for (const { url } of photos) {
    ...
    // Sanitize the URL before adding to HTML
    const sanitizedUrl = encodeURI(url);
    html += `<br><img src="${sanitizedUrl}" alt="Tweet image"/>`;
  }
  ...
  for (const { preview: url } of videos) {
    ...
    // Sanitize the URL before adding to HTML
    const sanitizedUrl = encodeURI(url);
    html += `<br><img src="${sanitizedUrl}" alt="Tweet video preview"/>`;
  }

  html = html.replace(/\n/g, '<br>');

  // CRITICAL: Sanitize the final HTML to prevent XSS attacks
  return sanitizeTweetHtml(html);
}
