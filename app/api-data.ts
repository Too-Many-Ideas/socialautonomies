...
export interface EndpointFieldInfo {
   ...
   * to include other representations of booleans as X's backend evolves.
   */
...
export type ApiRequestInfo<EndpointUrl> = EndpointFields<EndpointUrl> & {
  ...
  /**
   * Converts the request back into a URL to be sent to the X API.
   */
  toRequestUrl(): string;
};
...
/**
 * Parses information from a X API endpoint using an example request
