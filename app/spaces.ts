...
/**
 * Fetches available space topics from X.
 * @param auth The authentication object.
...
/**
 * Fetches available communities from X.
 * @param auth The authentication object.
...
/**
 * Logs in to X via Proxsee using the Periscope JWT to obtain a login cookie.
...
export async function fetchLoginTwitterToken(
    ...
    try {
        ...
        if (!data.cookie || !data.user) {
            throw new Error('X authentication failed, missing data.');
        }

        return data;
    } catch (error) {
        console.error('Error logging into X via Proxsee:', error);
        throw error;
    }
}